...
class SelfCheckAgent:
    ...
    def __init__(self, FAILED_TIMES_LIMIT=5, save_path=None):
        self.FAILED_TIMES_LIMIT = FAILED_TIMES_LIMIT
        self.code_error_count = 0
        self.save_path = save_path

    def self_check(
        self, obs, code_info=None, done=None, task_info=None, associative_memory=None
    ):
        """
        params:
            obs: observation
        ...
        if len(obs["event"]) > 0:
            ...
            for event in obs["event"]:
                if (
                    event["type"] == "chat"
                    and not event["message"].startswith(sender)
                    and obs["name"] in event["message"]
                ):
        ...
        if description == "Special Event":
            special_event_check = associative_memory.special_event_check(
                obs, task_info, code_timeout
            )
