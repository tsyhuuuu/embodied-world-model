...
class ServerManager:
    def __init__(self, max_memory="2G", wait_interval: float = 0.1, is_printing_server_info: bool = False):
        """
        ...
        self.another_server_is_running_filter = ["Perhaps a server is already running on that port"]
    ...
    def start(self):
        if not self.process or self.process.poll() is not None:
            command = "java -Xmx8G -jar fabric-server-mc.1.19-loader.0.14.18-launcher.0.11.2.jar nogui"
            args = shlex.split(command)

            self.process = subprocess.Popen(
                args, cwd=self.path, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE
            )
